"""Pot cooking-dynamics tests: status encoding, cook-time countdown, and the
onions-in-soup observation channel, all under `random_cook_time`/`random_pot_size`."""
import jax
...
def test_pot_encoding_consistency_with_sampled_dynamics():
    ...
    assert empty - full == max_onions
    # cook_time and max_onions_in_pot are sampled from fixed ranges
    # (see overcooked_env.py's random_cook_time/random_pot_size reset logic);
    # pin the *ranges*, not exact values — the exact draw shifts with any
    # unrelated change to the number/order of `jax.random.split` calls
    # upstream of this sample, so it isn't a meaningful invariant to snapshot.
    assert 10 <= full <= 30
    assert 1 <= max_onions <= 5
...
def test_observation_soup_channel_matches_max_onions_in_pot():
    ...
    for agent in env.agents:
        ...
        assert val == max_onions, (
            f"Agent {agent} sees {val} onions in soup, expected {max_onions}"
        )
