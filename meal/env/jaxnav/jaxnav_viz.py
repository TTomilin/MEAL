...
class JaxNavVisualizer(object):
    ...
    def init(self):
        self.env.init_render(self.ax, self.state_seq[0], self.obs_seq[0], lidar=self.plot_lidar, agent=self.plot_agent,
                             goal=False)
    ...
    def update(self, frame):
        ...
        if self.plot_path:
            for a in range(self.env.num_agents):
                ...
                self.env.map_obj.plot_agent_path(self.ax, self.path_seq[:plot_frame, a, 0],
                                                 self.path_seq[:plot_frame, a, 1])
            # self.ax.plot(self.path_seq[:frame, 0], self.path_seq[:frame, 1], color='b', linewidth=2.0, zorder=1)
        self.env.init_render(self.ax, self.state_seq[frame], self.obs_seq[frame], lidar=self.plot_lidar,
