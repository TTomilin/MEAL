...
from meal.env.jaxnav.jaxnav_utils import rot_mat
...
class PolygonMap(Map):
    ...
    def sample_map(self, key):
        ...
        bounds = jnp.array([
            ...
            [[0.0, self.map_size[1]], [self.map_size[0], self.map_size[1]], [self.map_size[0], self.map_size[1] - 0.15],
             [0.0, self.map_size[1] - 0.15]],
            [[self.map_size[0] - 0.15, 0.0], [self.map_size[0] - 0.15, self.map_size[1]],
             [self.map_size[0], self.map_size[1]], [self.map_size[0], 0.0]],
        ])

        # out of bounds constant
        oobounds = jnp.full((self.num_sides, 2), 1.0, dtype=jnp.float32) + jnp.array(
    ...
    def check_agent_map_collision(self, pos, coords):
        ...
        def _check_ac(p1, p2, pos):
            ...
            return jax.lax.cond(_in_map(p1, p2, pos, self.rad), lambda _: line_circle_collision(p1, p2, pos, self.rad),
                                lambda _: False, None)
