...
class Map(object):
        ...
        def _body_idx(val, key):
            ...
            def _cond_pos(val):
                ...
                map_collisions = jnp.any(
                    jax.vmap(self.check_circle_map_collision, in_axes=[0, None, 0])(pair[:, :2], map_data, radii))
                agent_collisions = jnp.any(
                    jax.vmap(_agent_collision, in_axes=[0, 1, None])(pair[:, :2], temp_case[:, :, :2],
                                                                     self.rad * self.start_pad))
                ...
                if self.valid_path_check:
                    valid_path = self.passable_check(pair[0, :2], pair[1, :2],
                                                     map_data)  # WARNING can make code too slow
                ...
                return self.check_agent_map_collision(pair[0], map_data, self.rad * self.start_pad) \
                    ...
                    | (jnp.linalg.norm(pair[0] - pair[1]) <= 2 * self.rad).astype(
                        jnp.bool_)  # \ | ~(self.rrt(key_rrt, map_data, pair[0], pair[1]))
    ...
    def rrt_star(self, key, map_data, start, goal):
        ...
        def _body_fun(val):
            ...
            # Check free space, line collision
            free_space = ~self.check_agent_map_collision(test_pos, map_data)
            circle_trans = self.check_agent_translation(test_pos, tree_pos,
                                                        map_data)  # NOTE do we need this for our valid check?
            ...
            new_node = jax.lax.select(valid, jnp.concatenate(
                [test_pos, jnp.array([parent_idx, cost, goal_just_reached.astype(jnp.int32)])]),
                                      jnp.zeros(tree.shape[1]))
            ...
            new_nodes = jnp.where(rewire[:, None], jnp.concatenate(
                [tree[parent_poss, :2], jnp.full((rrt_star_neighbours, 1), rrt_idx), new_cost[:, None],
                 tree[parent_poss, 4][:, None]], axis=1), tree[parent_poss])
    ...
    def plot_rrt_tree(self, ax, tree, goal_reached=False, goal=None, rrt_reward=None, name="env"):
        ...
        for n in range(tree.shape[0]):
            ...
            ax.plot([tree[n, 0], tree[parent_idx, 0]], [tree[n, 1], tree[parent_idx, 1]], c='gray', marker='+',
                    alpha=0.75)
