...
class GridMapCircleAgents(Map):
    ...
    def plot_map(self, ax: axes.Axes, map_grid: jnp.ndarray):
        ...
        ax.imshow(map_grid, extent=(0, map_grid.shape[1], 0, map_grid.shape[0]), origin="lower", cmap='binary',
                  zorder=0)
...
class GridMapPolygonAgents(GridMapCircleAgents):
    ...
    def check_agent_map_collision(self, pos, theta, map_grid, agent_coords=None):
        ...
        cut = jnp.any(
            jax.vmap(self._checkGrid,
                     in_axes=(None, None, 0, None))(scaled_coords_appended[:-1], scaled_coords_appended[1:], idx_pairs,
                                                    map_grid)
        )
    ...
    def get_agent_map_occupancy_mask(self, pos, theta, map_grid, agent_coords=None):
        ...
        cut = jax.vmap(self._checkGrid,
                       in_axes=(None, None, 0, None))(scaled_coords_appended[:-1], scaled_coords_appended[1:],
                                                      idx_pairs, map_mask)
    ...
    @partial(jax.jit, static_argnums=[0])
    def check_all_agent_agent_collisions(self, agent_positions: chex.Array, agent_theta: chex.Array,
                                         agent_coords=None) -> chex.Array:
        ...
        transformed_coords = jax.vmap(self.transform_coords, in_axes=(0, 0, None))(agent_positions,
                                                                                   agent_theta.squeeze(), agent_coords)
        ...
        orthog_edges = jax.vmap(_orthogonal, in_axes=(0))(edges.reshape((-1, 2))).reshape(
            (-1, 4, 2))  # NOTE assuming 4 sides
        ...
        axis_ranges_by_agent = jnp.stack(
            [jnp.min(axis_projections_by_agent, axis=2), jnp.max(axis_projections_by_agent, axis=2)], axis=-1)
        ...
        overlaps_matrix = jax.vmap(_calc_overlaps)(jnp.arange(self.num_agents),
                                                   axis_by_agent_range_by_agent)  # all overlaps for all vertex is a collision
...
INFLATE_IDX_OFFSETS_5 = jnp.concatenate([INFLATE_IDX_OFFSETS_3, jnp.array(
    [[2, 0], [2, 1], [2, 2], [1, 2], [0, 2], [-1, 2], [-2, 2], [-2, 1], [-2, 0], [-2, -1], [-2, -2], [-1, -2], [0, -2],
     [1, -2], [2, -2], [2, -1], ])], axis=0)
...
class GridMapBarn(GridMapPolygonAgents):
    ...
    def __init__(self,
        ...
        self.outer_idx = jnp.array([[i, j] for i in range(self.height) for j in range(self.width) if
