...
from meal.env.utils.spaces import Box, Discrete
...
class JaxNav(MultiAgentEnv):
    ...
    def __init__(self,
        ...
        self.lidar_angles = jnp.linspace(-jnp.pi * self.lidar_angle_factor, jnp.pi * self.lidar_angle_factor,
    ...
    def step_env(
        ...
        new_pos, new_theta, new_vel = self.update_state(agent_states.pos, agent_states.theta, agent_states.vel, actions,
                                                        update_state_valid)
        ...
        map_collisions = jax.vmap(self._map_obj.check_agent_map_collision, in_axes=(0, 0, None))(new_pos, new_theta,
                                                                                                 agent_states.map_data) * (
                                     1 - agent_states.done).astype(bool)
        if self.num_agents > 1:
            agent_collisions = self.map_obj.check_all_agent_agent_collisions(new_pos, new_theta) * (
                        1 - agent_states.done).astype(bool)
        else:
    ...
    def _lidar_sense(self, idx: int, state: State) -> chex.Array:
        ...
        lidar_hits = point_fn(beam_coords.reshape(-1, 2), state.map_data).reshape(beam_coords.shape[0],
                                                                                  beam_coords.shape[1], -1)
    ...
    def _get_obs(self, state: State) -> chex.Array:
        ...
        def _observation(idx: int, state: State) -> jnp.ndarray:
            ...
            vel_goal = jnp.concatenate(
                [vel_obs, goal_dist[None], goal_orient[None], jnp.array([rew_lambda]).reshape(1)])
    ...
    def update_state(self, pos: chex.Array, theta: float, speed: chex.Array, action: chex.Array,
                     done: chex.Array) -> chex.Array:
    ...
    def compute_reward(
        ...
        rw = jax.lax.select(jnp.abs(act[1]) > self.lim_w, self.weight_w * jnp.abs(act[1]),
                            0.0)  # angular velocity magnitue penalty
        rt = self.dt_rew * (1 - (old_goal_reached | old_move_term))  # time penalty
        rl = jnp.any(self.unnormalise_lidar(obs[:self.lidar_num_beams]) <= (
                    self.lidar_thresh + self.rad)) * self.lidar_rew  # lidar proximity reward
    ...
    def unnormalise_obs(self, obs_batch: chex.Array) -> chex.Array:
        ...
        vel_obs = (obs_batch[:, self.lidar_num_beams:self.lidar_num_beams + 2] + jnp.array([0.5, 0.0])) * jnp.array(
    ...
    def action_space(self, agent=None):
        # Homogeneous agents: just return per-agent space
    ...
    def plot_lidar(self, ax: axes.Axes, obs: Dict, state: State, num_to_plot: int = 10):
        ...
        def lidar_scatter(
            ...
            """
            ranges = self.unnormalise_lidar(
                lidar_ranges)  # (lidar_ranges+0.5)*self.lidar_max_range  # correct normalisation
    ...
    def init_render(self,
        ...
        if agent:
            self.map_obj.plot_agents(ax, state.pos, state.theta, state.goal, state.done, plot_line_to_goal=goal,
                                     colour_agents_by_idx=colour_agents_by_idx)
        if lidar:
