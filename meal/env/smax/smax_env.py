...
from meal.env.smax.distributions import (
...
class SMAX(MultiAgentEnv):
    ...
    def _update_dead_agents(self, state: State):
    ...
    def _decode_discrete_actions(
        ...
        actions = jnp.asarray(actions).reshape(-1)  # ensure shape (num_agents,)
    ...
    def render_state(self, ax, state: State, step_label: str = ""):
        """Render a single state onto a matplotlib axis."""
        ...
        ax.set_yticks([])
        if step_label:
            ax.set_title(step_label, color="white", fontsize=8)
        ...
        for i in range(self.num_allies):
            if state.unit_alive[i]:
                ...
                r = float(self.unit_type_radiuses[state.unit_types[i]])
                c = Circle(pos, r, facecolor=ally_color, edgecolor="white",
                           linewidth=0.8, zorder=2)
                ax.add_patch(c)
                ax.text(pos[0], pos[1],
                        self.unit_type_shorthands[int(state.unit_types[i])],
        ...
        for i in range(self.num_enemies):
            ...
            if state.unit_alive[idx]:
                ...
                r = float(self.unit_type_radiuses[state.unit_types[idx]])
                c = Circle(pos, r, facecolor=enemy_color, edgecolor="white",
                           linewidth=0.8, zorder=2)
                ax.add_patch(c)
                ax.text(pos[0], pos[1],
                        self.unit_type_shorthands[int(state.unit_types[idx])],
                        ha="center", va="center", fontsize=5, color="black",
                        fontweight="bold", zorder=3)
