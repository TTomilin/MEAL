...
from meal.visualization.rendering.state_visualizer_old import StateVisualizer
...
class OvercookedVisualizer:
    ...
    def __init__(self, num_agents: int = 2, use_old_rendering: bool = False,
        ...
        # Initialize the state visualizer if using the new rendering
        if not self.use_old_rendering:
            self.state_visualizer = StateVisualizer(tile_size=TILE_PIXELS)
    ...
    def animate(self, state_seq, agent_view_size, task_idx, task_name, exp_dir, tile_size: int = TILE_PIXELS,
        ...
        Args:
            ...
            agent_view_size: Agent view size for padding calculation
            task_idx: Task index for naming
            task_name: Task name for naming
            exp_dir: Export directory
            tile_size: Size of each tile in pixels
            dynamic_grid_size: If True, automatically handle different grid sizes by using max dimensions
        """
        import os
        import pygame
        from PIL import Image

        # Detect grid sizes and calculate appropriate padding
        if dynamic_grid_size:
            grid_sizes = self._detect_grid_sizes(state_seq, agent_view_size)
            max_height, max_width = self._calculate_max_grid_size(grid_sizes)
            # Use the maximum padding needed across all grids
            max_padding = max(gs['padding'] for gs in grid_sizes.values())
            padding = max_padding
        else:
            padding = agent_view_size - 1  # 5→4 because map has +1 outer wall
        ...
        if self.use_old_rendering:
            def get_frame(state, state_idx=None):
                # Check if state is a LogEnvState (has env_state attribute)
                env_state = state.env_state if hasattr(state, 'env_state') else state

                if dynamic_grid_size and state_idx is not None:
                    # Use individual state padding for dynamic grid sizes
                    state_padding = grid_sizes[state_idx]['padding']
                    grid = np.asarray(env_state.maze_map[state_padding:-state_padding, state_padding:-state_padding, :])

                    # Pad grid to maximum size if needed
                    if max_height is not None and max_width is not None:
                        grid = self._pad_grid_to_size(grid, max_height, max_width)
                else:
                    # Use global padding for uniform grid sizes
                    grid = np.asarray(env_state.maze_map[padding:-padding, padding:-padding, :])
            ...
            frames = [get_frame(state, i) for i, state in enumerate(state_seq)]
        else:
            ...
            for state_idx, state in enumerate(state_seq):
                # Check if state is a LogEnvState (has env_state attribute)
                env_state = state.env_state if hasattr(state, 'env_state') else state
                ...
                if dynamic_grid_size:
                    ...
                    # Calculate padding offsets for agent position adjustment
                    original_height, original_width = grid.shape[:2]
                    pad_offset_x = 0
                    pad_offset_y = 0
                    ...
                    if max_height is not None and max_width is not None:
                        if original_height < max_height or original_width < max_width:
                            # Calculate padding offsets to center the grid
                            pad_height = max_height - original_height
                            pad_width = max_width - original_width
                            pad_offset_y = pad_height // 2
                            pad_offset_x = pad_width // 2
                ...
                for y in range(grid.shape[0]):
                    for x in range(grid.shape[1]):
                        if grid[y, x, 0] == OBJECT_TO_INDEX['agent']:
                            ...
                            # Adjust position based on padding offset when grid is padded
                            pos_x = x + pad_offset_x
                            pos_y = y + pad_offset_y
                            agent_data.append((agent_idx, (pos_x, pos_y)))
                ...
                for agent_idx, pos in agent_data:
                    if hasattr(env_state, 'agent_dir_idx') and agent_idx < len(env_state.agent_dir_idx):
                        # Convert environment direction index to visualization direction tuple
                        # Convert JAX array to int before using as dictionary key
                        dir_idx = int(env_state.agent_dir_idx[agent_idx])
                        orientation = ENV_DIR_IDX_TO_VIZ_DIR[dir_idx]
                    else:
                    ...
                    # Create a player with appropriate held object based on inventory
                    held_object = None
                    if hasattr(env_state, 'agent_inv') and agent_idx < len(env_state.agent_inv):
                        held_object = self._create_held_object_from_inventory(int(env_state.agent_inv[agent_idx]))
                    players.append(MockPlayer(position=pos, orientation=orientation, held_object=held_object))
        ...
        file_path = f"{exp_dir}/{task_name}.gif"
        ...
        if wandb.run is not None:
            wandb.log({task_name: wandb.Video(file_path, format="gif")})
    ...
    def _detect_grid_sizes(self, state_seq, agent_view_size):
        """
        Detect different grid sizes in the state sequence.

        Args:
            state_seq: List of states to analyze
            agent_view_size: Agent view size for padding calculation

        Returns:
            Dict mapping state indices to grid size information
        """
        grid_sizes = {}
        ...
        for i, state in enumerate(state_seq):
            ...
            # Get the full maze_map dimensions
            maze_shape = env_state.maze_map.shape
            full_height, full_width = maze_shape[0], maze_shape[1]

            # Calculate padding based on agent_view_size
            padding = agent_view_size - 1

            # Calculate actual grid dimensions after padding removal
            grid_height = full_height - 2 * padding
            grid_width = full_width - 2 * padding
    ...
    def _calculate_max_grid_size(self, grid_sizes):
        """
        Calculate the maximum grid size needed to accommodate all states.

        Args:
            grid_sizes: Dict from _detect_grid_sizes

        Returns:
            Tuple of (max_height, max_width) for the actual grid content
        """
        if not grid_sizes:
            return 0, 0

        max_height = max(gs['grid_height'] for gs in grid_sizes.values())
        max_width = max(gs['grid_width'] for gs in grid_sizes.values())

        return max_height, max_width

    def _pad_grid_to_size(self, grid, target_height, target_width):
        """
        Pad a grid to match target dimensions by adding empty space around it.

        Args:
            grid: The grid to pad (H, W, C)
            target_height: Target height
            target_width: Target width

        Returns:
            Padded grid with target dimensions
        """
        current_height, current_width = grid.shape[:2]
        ...
        # Distribute padding evenly (center the original grid)
        pad_top = pad_height // 2
        pad_bottom = pad_height - pad_top
        pad_left = pad_width // 2
        pad_right = pad_width - pad_left

        # Create padding with empty space (assuming 0 represents empty space)
        if len(grid.shape) == 3:
