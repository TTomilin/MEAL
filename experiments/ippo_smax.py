"""
IPPO training script for the SMAX continual learning benchmark.

Task diversity is driven by unit composition variation: ally and enemy teams are
assigned independently sampled unit types (marine/marauder/stalker/zealot/zergling/
hydralisk) from the 6 available types. Each of the 100 tasks presents a unique
matchup, requiring fundamentally different strategies (ranged vs melee, tank vs
glass-cannon, etc.).

Usage:
    conda run -n meal python experiments/ippo_smax.py \\
        --cl_method ft --seq_length 10 --num_allies 5 --num_enemies 5 \\
        --seed 42 --use_wandb false
"""
...
def main():
    ...
    envs = make_smax_sequence(
        ...
        seed=seed,
        num_allies=cfg.num_allies,
        num_enemies=cfg.num_enemies,
        max_steps=cfg.max_steps,
        enemy_shoots=cfg.enemy_shoots,
    )
        ...
        def _update_step(runner_state, _):
            ...
            def _update_epoch(update_state, _):
                ...
                (train_state, cl_state, rng), loss_information = jax.lax.scan(
                    _update_minbatch, (train_state, cl_state, rng), minibatches
                )
    ...
    def loop_over_envs(rng, train_state, cl_state, envs):
        ...
        for task_idx, (task_rng, env) in enumerate(zip(env_rngs, envs)):
            ...
            if cfg.save_checkpoints:
                ...
                path = (
                    f"{repo_root}/checkpoints/smax/{cfg.cl_method}"
