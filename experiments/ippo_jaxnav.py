...
from experiments.evaluation_jaxnav import evaluate_all_envs, make_eval_fn
...
def main():
    ...
    def loop_over_envs(rng, train_state, cl_state, envs):
        ...
        for task_idx, (rng, env) in enumerate(zip(env_rngs, envs)):
            # --- Task Training ---
            print(f"Training on environment: {task_idx} - {env.map_id}")
            ...
            path = f"{repo_root}/checkpoints/jaxnav/{cfg.cl_method}/{run_name}/model_env_{task_idx + 1}"
