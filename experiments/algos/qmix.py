...
from experiments.utils_vdn import (
    CustomTrainState, eps_greedy_exploration,
...
def make_q_importance_fn(reset_switch, step_switch, network, agents, use_cnn: bool,
    ...
    """Importance estimation for Q-networks (used by EWC/MAS). Operates on Q-network
    params only; called with train_state.params["q"]."""
    num_agents = len(agents)
...
class QMIX(OffPolicyAlgo):
    def single_task_cl_method(self) -> str:
        return "ft"  # matches original qmix.py's exact casing (vdn.py used "FT")
    ...
    def init_network(self):
        ...
        train_state = CustomTrainState.create(
            apply_fn=network.apply, params=combined_params,
            target_network_params=combined_params, tx=tx,
        )

        self.rng = rng
        self.network = network
        self.mixing_network = mixing_network
        self.q_network_params = q_network_params
    ...
    def build_train_on_environment(self):
        ...
        mixing_network = self.mixing_network
        tx = self.tx
        cl = self.cl
        agents = self.agents
        num_agents = self.num_agents
        ...
        def train_on_environment(rng, train_state, train_env, cl_state, env_idx):
            ...
            def _update_step(runner_state, _):
                ...
                q_next_online = jax.vmap(
                    lambda p, o: network.apply(p, o, env_idx=env_idx), in_axes=(None, 0)
                )(train_state.params["q"], nxt_b)
                best_actions = jnp.argmax(q_next_online, axis=-1, keepdims=True)
                q_next_target = jax.vmap(
                    lambda p, o: network.apply(p, o, env_idx=env_idx), in_axes=(None, 0)
                )(tgt_q_params, nxt_b)
                q_next_max = jnp.take_along_axis(q_next_target, best_actions, axis=-1).squeeze(-1)
                q_next_max_T = q_next_max.T

                q_total_next = mixing_network.apply(tgt_mix_params, q_next_max_T, ngs_flat)
                ...
                def _learn_minibatch(train_state, mb_indices):
                    ...
                    def _loss_fn(combined_params):
                        ...
                        chosen_q_T = chosen_q.T

                        q_total = mixing_network.apply(mix_params, chosen_q_T, mb_gs)
                        td_loss = jnp.mean((q_total - mb_tgt) ** 2)

                        cl_penalty = cl.penalty(q_params, cl_state, cfg.reg_coef) if isinstance(cl,
                    ...
                    if cfg.cl_method.lower() == "agem":
                        ...
                        for _t in range(_max_tasks):
                            ...
                            _t_grads = compute_qmix_memory_gradient(
                                network, mixing_network, train_state.params, train_state.target_network_params,
                                cfg.gamma, _t_obs, _t_global, _t_acts, _t_rews, _t_nobs, _t_next_global, _t_dones,
                                env_idx=_t,
                            )
                ...
                # Soup/shaped-reward metrics are Overcooked-specific; only computed when
                # the active env adapter provides them (see equivalent block in vdn.py).
                    ...
                    def log_metrics(metrics, update_step):
                        if cfg.evaluation:
                            avg_rewards, avg_soups = evaluate_all_envs(
                                cl_state, eval_rng, train_state.params["q"], cfg.seq_length, self.evaluate_env
                            )
            ...
            if cfg.cl_method.lower() in ("agem", "er_ace"):
                ...
                cl_state = update_qmix_agem_memory(
                    cl_state, env_idx, _obs_b, _gs, _act_b, _rew, _nxt_b, _ngs, _don,
                )
    ...
    def loop_over_envs(self, rng, train_state, cl_state, train_on_environment):
        ...
        for task_idx, (env_rng, train_env, env) in enumerate(
            ...
            importance = self.importance_fn(train_state.params["q"], task_idx, rng)
            cl_state = self.cl.update_state(cl_state, train_state.params["q"], importance)
            ...
            self.save_params(
                path, train_state, env_kwargs=getattr(env, "layout", None), layout_name=env_name, config=cfg,
                extra_fields={"mixing_embed_dim": cfg.mixing_embed_dim},
            )
    ...
    def run(self):
        ...
        if self.cfg.cl_method.lower() in ("agem", "er_ace"):
            cl_state = init_qmix_agem_memory(
                ...
                state_dim=self.state_dim, max_tasks=self.cfg.seq_length,
            )
        else:
            cl_state = init_cl_state(self.q_network_params, False, not self.cfg.use_multihead, self.cl, self.cfg)
